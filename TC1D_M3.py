...
def echo_model_info(
    ...
    ero_models = {
        ...
        8: "Step-function+linear rate change",
        9: "Lava+Step-function+linear rate change"
    }
...
#######Sean Changed to include adiabat_m
...
def lava_growth(params, x, xstag, temp_prev, moho_depth):
    """Defines temperatures and material properties for ero_types 4 and 5."""
    ...
    # Adjust temperatures depending on erosion model type
    if params["ero_type"] == 9:
        # Reassign temperatures
        for ix in range(params["nx"]):
            if ix >= ref_index:
                temp_prev[ix] = initial_temps[ix - ref_index]
        moho_depth += kilo2base(0.5)
        temp_prev[1]=params["ero_option8"][0]
...
, P, Q, R, A2,t, start,stop,flux):
    ...
    N=nx
    b[nx - 1] = temp_base
    vlx=np.zeros(len(vx), float)
    vlx[0:int(N/5)]+=flux*1e-10
    ...
    if t>start and t<stop:
        for ix in range(1, nx - 1):
            a_matrix[ix, ix - 1] = (
                -(rho[ix] * cp[ix] * -(vx[ix]+vlx[ix])) / (2 * dx) - k[ix - 1] / dx**2
            )
            a_matrix[ix, ix] = (rho[ix] * cp[ix]) / dt + k[ix] / dx**2 + k[ix - 1] / dx**2
            a_matrix[ix, ix + 1] = (rho[ix] * cp[ix] * -(vx[ix]+vlx[ix])) / (2 * dx) - k[ix] / dx**2
    ...
    '''
    P[0:N-1, 0:N-1]=np.diag(((
        -(rho[1:N-1] * cp[1:N-1] * -(vx[1:N-1]+vlx[1:N-1])) / (2 * dx) - k[0:N-2] / dx**2
    )), -1)
    #Q=np.zeros([1:N-2, 1:N-2],float)
    Q[1:N-1, 1:N-1]=np.diag(((rho[1:N-1] * cp[1:N-1]) / dt + k[1:N-1] / dx**2 + k[0:N-2] / dx**2), 0)
    R[1:N, 1:N]=np.diag(((rho[1:N-1] * cp[1:N-1] * -(vx[1:N-1]+vlx[1:N-1])) / (2 * dx) - k[1:N-1] / dx**2),1)
    A2=P+R+Q
    A2[0,0]=1
    A2[-1,-1]=1
    a_matrix=A2
    b[1:N-1] = heat_prod[1:N-1] + ((rho[1:N-1] * cp[1:N-1]) / dt) * temp_prev[1:N-1]
    '''
    temp = solve(a_matrix, b)
    return temp
...
def calculate_erosion_rate(
    ...
    #Sean modification changing erotype 2 to have initial constant then 
    ...
    # 2 constant phases of erosion followed by gradual linearly decreasing phase
    elif params["ero_type"] == 8:
        init_rate1 = mmyr2ms(params["ero_option1"])
        init_rate2= mmyr2ms(params["ero_option3"])
        rate_change_start1 = myr2sec(params["ero_option2"])
        rate_change_start2 = myr2sec(params["ero_option4"])
        final_rate = mmyr2ms(params["ero_option5"])
        rate_change_end = t_total
        
        if current_time < rate_change_start1:
            vx_array[:] = init_rate1
            vx_surf = vx_array[0]
        elif rate_change_start1 < current_time < rate_change_start2:
            vx_array[:] = init_rate2 
            vx_surf = vx_array[0]
        elif rate_change_start2 < current_time < rate_change_end:
            vx_array[:] = init_rate2 + (current_time - rate_change_start2) / (
                rate_change_end - rate_change_start2
            ) * (final_rate - init_rate2)
            vx_surf = vx_array[0]
        else:
            vx_array[:] = final_rate
            vx_surf = vx_array[0]
        vx_max = max(init_rate1, init_rate2, final_rate)

    # 2 constant phases of erosion followed by gradual linearly decreasing phase
    elif params["ero_type"] == 9:
        init_rate1 = mmyr2ms(params["ero_option1"])
        init_rate2= mmyr2ms(params["ero_option3"])
        rate_change_start1 = myr2sec(params["ero_option2"])
        rate_change_start2 = myr2sec(params["ero_option4"])
        final_rate = mmyr2ms(params["ero_option5"])
        rate_change_end = t_total
        lava_time1 = myr2sec(params["ero_option6"][0])
        lava_time2 = myr2sec(params["ero_option7"][0])
        lava_thickness = 500.0
        ...
        if current_time < rate_change_start1:
            ...
            vx_surf = vx_array[0]
        vx_max = max(init_rate1, init_rate2, final_rate)
        #if abs(lava_time1 - current_time) < 1.0e-6 or abs(lava_time2 - current_time) < 1.0e-6:
            #vx_array[:] -= mmyr2ms(500.0*1000.0/5000.0) 
            #vx_surf = vx_array[0]
...
def calculate_exhumation_magnitude(
    ...
    if ero_type == 1:
       ...
       magnitude = myr2sec(ero_option1) * mmyr2ms(ero_option2)
       # Handle case that ero_option4 is not specified (i.e., linear increase ends at end of simulation)
       magnitude += myr2sec(ero_option3) * (mmyr2ms(ero_option4)-mmyr2ms(ero_option2))
       rate_change_end = t_total
       
       magnitude += (rate_change_end - myr2sec(ero_option4)) * (
           0.5 * abs(mmyr2ms(ero_option3) - mmyr2ms(ero_option5)) + mmyr2ms(ero_option5)
       )
       #magnitude += (t_total - rate_change_end) * mmyr2ms(ero_option3)
       magnitude /= 1000.0
    elif ero_type == 9:
...
def run_model(params):
    ...
    ## create arrays for the matrix diagonals
    P = np.zeros(params["nx"])
    Q = np.zeros(params["nx"])
    R = np.zeros(params["nx"])
    A2 = np.zeros([params["nx"],params["nx"]])
    ...
    ##########Sean Added This
    #print("removal",params["removal_fraction"])
    #if (params["removal_fraction"] > 0.0) and (params["New_param"][0] < 1e-6):
    #    for ix in range(params["nx"]):
    #        if x[ix] > (max_depth - removal_thickness):
    #            temp_prev[ix] = params["temp_base"] + (x[ix] - max_depth) * 0.0
    #    delaminated = False
        #Sean=1
    # Set plot parameters if plotting requested
    ...
    for j in range(num_pass):
        ...
        if num_pass == 1:
            ...
            while curtime < t_total:
                ...
                if params["ero_type"] == 90 :
                    if abs(myr2sec(params["ero_option6"][0])-curtime) < dt:
                        print('this is happening')
                        depths[move_particles] += 5000.0#(vx_hist[:nt_now] * dt).sum() - (500.0)
                    if abs(myr2sec(params["ero_option7"][0])-curtime) < 0.0e-1:
                        depths[move_particles] += 5000.0
                ...
                #if params["ero_type"] == 9 :
                #    if params["ero_option6"][0] > 0.0:
                ##        depths[i] += 50.0#(vx_hist[:nt_now] * dt).sum() - (500.0)
                #    if params["ero_option7"][0] > 0.0:
                #        depths[i] += 50.0 #= (vx_hist[:nt_now] * dt).sum() - (500.0)
            ...
            if params["ero_type"] == 9 :
                if params["ero_option6"][0] > 0.0:
                    depths[i] -= 499.0#(vx_hist[:nt_now] * dt).sum() - (500.0)
                if params["ero_option7"][0] > 0.0:
                    depths[i] -= 499.0 
            curtime = 0.0
            idx = 0
        ...
        while curtime < t_total:
            ...
            if (
                ...
                in_lava_interval = (
                    ((curtime - (dt / 2)) / myr2sec(1)
                    <= params["ero_option6"][0]
                    < (curtime + (dt / 2)) / myr2sec(1))
                    or ((curtime - (dt / 2)) / myr2sec(1)
                    <= params["ero_option7"][0]
                    < (curtime + (dt / 2)) / myr2sec(1))
                )
                if in_lava_interval:
                    (
                        ...
                        heat_prod,
                        alphav,
                    ) = lava_growth(params, x, xstag, temp_prev, moho_depth)
                    depths[i] += kilo2base(0.5)
                    
              
            #Sean_changes-set surface changes
            # f abs(curtime-dt*1000)<0.00001 or abs(curtime-dt*2000)<0.000001:
            #     temp_prev[1]=1300.0
            # Set mantle temperatures to adiabat if in removal interval
            if (params["removal_fraction"] > 0.0) and (params["Mixing_time"][0]>0.0):
                
                end_time=float(params["Mixing_time"][0])
                in_removal_interval = (
                ...
                if curtime / myr2sec(1) < params["removal_end_time"]:
                    ...
                    removal_thickness = (
                    params["removal_fraction"]* (max_depth - moho_depth)
                    )
                #print(removal_thickness)
                #if curtime / myr2sec(1) < params["removal_end_time"]:
                #    removal_thickness = (
                #        (curtime / myr2sec(1) - params["removal_start_time"])
                #        / (params["removal_end_time"] - params["removal_start_time"])
                ...
                #    )
                #    print(removal_thickness)
            #elif (params["removal_fraction"] > 0.0) and (not delaminated):
            #    # Episodic removal
            ...
            #        )
                ...
                ## Sean edit Well mixed removal New_param must be larger than end time
                ...
                if in_removal_interval:
                    ...
                    if removal_thickness > (max_depth - moho_depth):
                        moho_depth = (max_depth - removal_thickness)
                    ###Sean mods here to delaminated=True
                    if params['removal_end_time']<params["Mixing_time"][0]:
                        if curtime / myr2sec(1) > params["Mixing_time"][0]:
                            delaminated = True
                    else:
                        if curtime / myr2sec(1) > params["removal_end_time"]:
                            delaminated = True
            ...
            ### SK changes
            if params["param11"][0]>0.0e-6 and (params["param11"][0] < (curtime / myr2sec(1)) < params["param12"][0]):
                ...
                rho, cp, k, heat_prod, lab_depth = update_materials(
                    ...
                    params["k_mantle"],
                    k,
                    micro2base(params["heat_prod_crust"]+0.06),
                    micro2base(params["heat_prod_mantle"]),
                    heat_prod,
                    temp_adiabat,
                    temp_prev,
            ...
            if params["implicit"]:
                if params["param13"][0]>0.0e-6 and (params["param13"][0] < curtime / myr2sec(1) < params["param14"][0]):
            ...
            if j == num_pass - 1:
                ...
                interp_temp_new = interp1d(x, temp_new, fill_value="extrapolate")
                # Calculate lithostatic pressure
                pressure = calculate_pressure(density_new, dx)
                interp_pressure = interp1d(x, pressure, fill_value="extrapolate")
